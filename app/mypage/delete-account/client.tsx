...
export default function DeleteAccountClient() {
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setBusy(true);
    apiDeleteAccount({ password: input.password }, { token: jwt?.accessToken }).then(
      (s) => {
        if (s.status === 'SUCCESS') {
  ...
  return (<DeleteAccountForm submitting={busy} handleInput={handleInputBuilder(input, setInput)} input={input} handleSubmit={handleSubmit} />)
