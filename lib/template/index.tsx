...
export default function ErrorTemplate({ title, subtitle, back }: { title: string, subtitle: string, back?: string }) {
  return <main>
    <HStack className="pt-24 pb-24 pl-8 pr-8 text-center justify-center">
      ...
      <span className="text-xs  text-light-back-4 ">이 오류가 버그인 것 같다면 <Link className="text-primary-500 underline" href={process.env.NEXT_PUBLIC_BUG_REPORT || ''}>버그리포트</Link>를 부탁드려요.</span>
    </HStack>
    </main>
}